...
def add_constraints(data, model):
    ...
    _loss_model_constraint(model, base, peak)
...
def add_peak_cutting_objective(data, model):
    ...
    maxpower = data.objective.maxvalue

    def peak_cutting(mod, ii):
        return signal[ii] - (mod.base[ii] + mod.peak[ii]) <= maxpower

    model.con_peak_cutting = pe.Constraint(model.ind, rule=peak_cutting)
...
def add_throughput_objective(data, model):
    ...
    model.con_throughput = pe.Constraint(expr=sum(model.deltaplus[ii]*dtime[ii]
                                                  for ii in model.ind))
...
def _add_power_vars(model, base, peak):
    ...
    model.base = pe.Var(model.ind, bounds=(base.minpower, base.maxpower))
    model.peak = pe.Var(model.ind, bounds=(peak.minpower, peak.maxpower))

    # Positive and negative part of base and peak power
    model.baseplus = pe.Var(model.ind, bounds=(0, base.maxpower))
    model.baseminus = pe.Var(model.ind, bounds=(base.minpower, 0))
    model.peakplus = pe.Var(model.ind, bounds=(0, peak.maxpower))
    model.peakminus = pe.Var(model.ind, bounds=(peak.minpower, 0))
    ...
    model.inter = pe.Var(model.ind, bounds=(max(base.minpower, peak.minpower),
                                            min(base.maxpower, base.minpower)))
...
def _add_binary_vars(model):
    ...
    model.bininerjupper = pe.Var(within=pe.Binary)
...
def _bounds_for_inter_constraint(model, base, peak):
    ...
    def bounds_inter_base(mod, ii):
        return base.minpower, mod.base[ii] + mod.inter[ii], base.maxpower

    def bounds_inter_peak(mod, ii):
        return peak.minpower, mod.peak[ii] - mod.inter[ii], peak.maxpower
...
def _loss_model_constraint(model, base, peak):
    ...
    def base_losses(mod, ii):
        efficiency_losses = (mod.baseplus[ii] * base.efficiency +
                             mod.baseminus[ii] / base.efficiency)
        if ii is 0:
            discharge_losses = -mod.baseenergyinit * base.discharge
        else:
            discharge_losses = -mod.baseenergy[ii - 1] * base.discharge
        return mod.pwridealbase[ii] == efficiency_losses + discharge_losses

    def peak_losses(mod, ii):
        efficiency_losses = (mod.peakplus[ii] * peak.efficiency +
                             mod.peakminus[ii] / peak.efficiency)
        if ii is 0:
            discharge_losses = -mod.peakenergyinit * peak.discharge
        else:
            discharge_losses = -mod.peakenergy[ii - 1] * peak.discharge
        return mod.pwridealpeak[ii] == efficiency_losses + discharge_losses
...
def _binary_bounds_constraint(model, base, peak):
    ...
    def bin_bound_base(mod, ii):
        return (base.minpower*mod.binbaselower[ii],
                mod.base[ii],
                base.maxpower*mod.binbaseupper[ii])

    def bin_bound_peak(mod, ii):
        return (peak.minpower*mod.binpeaklower[ii],
                mod.peak[ii],
                peak.maxpower*mod.binpeakupper[ii])

    inter_minpower = max(base.minpower, peak.minpower)
    inter_maxpower = min(base.maxpower, peak.maxpower)
